...
function mobileHandler(){
    mobileState += 1
    mobileState = mobileState%5
    if(mobileState == 0){
        $("#not").css({top: 0, left: 0, position: 'sticky'});
    }
    else{
        let state = mobile_states[mobileState]
        let config = {position: 'absolute'}
        let body = $("body")
        let button = $("#not")
        if(state.top == 1){
            config.top = button.height()
        }
        else{
            config.top = body.height() - button.height()*2
        }
        if(state.left == 1){
            config.left = button.width()
        }
        else{
            config.left = body.width() - button.width()*2
        }
        $("#not").css(config);
    }
}
